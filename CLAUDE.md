...
**Limite de session** : l'orchestrateur maintient un compteur de phases/agents **producteurs** (ceux qui écrivent des fichiers) et alerte l'utilisateur quand la session risque de dégénérer (voir orchestrator.md — Compteur de session obligatoire). Seuils : ALERTE JAUNE après 4 phases / 12 agents producteurs, ALERTE ROUGE après 6 phases / 18 agents producteurs. Les agents de consultation (@elon, @moi, @ia en mode audit) ne comptent pas. Un projet complet doit être découpé en plusieurs sessions.
