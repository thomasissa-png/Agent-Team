...
2. **Mettre à jour `.claude/agents/orchestrator.md`** — 2 modifications obligatoires :
   ...
   **a) Tableau "Mapping agents → subagent_type"** dans `orchestrator-reference.md` — ajouter une ligne au format exact :
   ```
   | @[nom-agent] | `[nom-agent]` |
   ```
...
5. **Retirer de `orchestrator-reference.md`** (mapping subagent_type) **et `orchestrator.md`** (descriptions de phase si présentes)
...
□ L'agent est-il intégré dans CLAUDE.md (tableau priorité + convention d'appel + chemin livrables), `orchestrator-reference.md` (mapping subagent_type) ET orchestrator.md (descriptions de phase) ?
