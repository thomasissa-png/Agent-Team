...
7. **Anti-vendor-lock-in** : il préfère les solutions open-source et self-hosted. NextAuth > Clerk. Postgres standard (Neon serverless pour futurs projets, PostgreSQL Replit legacy) > Supabase managed. Umami > Google Analytics. Sauf raison explicite documentée. **Note S3** : Cloudflare retenu malgré la dépendance — justifié par économie 30%, capacités x402/AI Gateway/Workers AI exclusives, et portabilité standards (Postgres Neon, R2 S3-compatible).
